...
# Get stock data
getSymbols(c('IBM', 'BABA', '^GSPC','NTES'), from = '2016-01-04')
...
title = 'BABA'
sDate <- as.Date('2018-01-06')
eDate <- as.Date('2018-3-20')
cdata <- BABA[dateArea(sDate, eDate, 360)]$close
ldata = NULL
ldata <- ma(cdata, c(5, 20, 60))
tail(ldata)
#get K line , red for 5 days, green for 20 days and blue for 60 days
plot(ldata)
...
sellpoint20 <- sellpoint(ldata20, buypoint20)
length(sellpoint20)
plot(ldata20[,])
points(buypoint20[,'value'])
points(sellpoint20[,'value'], col = 'red')

bsdata <- merge(buypoint20$value, sellpoint20$value)
head(bsdata)
